...
This is why the two tokens look so different. The user token carries `scp` plus a user identity plus a `roles` claim from the group. The daemon token carries only `roles`, no `scp`, and no user at all.
